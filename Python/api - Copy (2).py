...
def index6(Colour='',Aspect='',Nitrite='',Albumin='',Suger='',Acetone='',Bile_Salts='',Bile_Pigments='',
    ...
    result = CMEAnalyzes.UrineTest(Colour,Aspect,Nitrite,Albumin,Suger,Acetone,Bile_Salts,Bile_Pigments,
             Urobilinogen,Leukocyte_estrease,Epithelial_Cells, Casts,Mucus,Ova,
                  Crystals,Yeast_Cells,Trichomonas_vaginalis,Volume,Reaction,
                  Specific_Gravity_in_urine,RBCs,Pus_Cells)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index7(Cholesterol=0.0,Triglyceride=0.0,HDL_C=0.0,LDL_C=0.0):
    result = CMEAnalyzes.LipidProfile(Cholesterol,Triglyceride,HDL_C,LDL_C)
    return json.dumps(result, ensure_ascii=False).encode('utf8')

@app.route('/DiabetesAnalyzes/<FBS>')
def index8(FBS=0.0):
    result = DiabetesAnalyzes.FastingBloodSuger(FBS)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index9(Fasting_Blood_Glucose=0.0,Glucose_After_2_Hours=0.0,Haemoglobin_A1C=0.0):
    result = DiabetesAnalyzes.DiabeticProfile(Fasting_Blood_Glucose,Glucose_After_2_Hours,Haemoglobin_A1C)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index10(state='',Fasting_Blood_Glucose=0.0,Glucose_After_30_min=0.0,Glucose_After_1_Hour=0.0,
    ...
    result = DiabetesAnalyzes.GlucoseCurve(state,Fasting_Blood_Glucose,Glucose_After_30_min,Glucose_After_1_Hour,
                     Glucose_After_2_Hours,Glucose_After_3_Hours)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index11(gender='',Normoalbuminuria=0.0,Microalbuminuria=0.0,Macroalbuminuria=0.):
    result = DiabetesAnalyzes.Microalbuminuria(gender,Normoalbuminuria,Microalbuminuria,Macroalbuminuria)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index12(ALt_SGPT=0.0,AST_SGOT=0.0,ALP=0.0,Albumin=0.0,Total_protein=0.0,Bilirubin=0.0,GGT=0.0,LD=0.0,PT=0.0):
    result = LiverFunctionTest.LiverFunctionTest(ALt_SGPT,AST_SGOT,ALP,Albumin,Total_protein,Bilirubin,GGT,LD,PT)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index13(gender='',Glucose=0.0,BUN=0.0,Creatinine=0.0,Calcium=0.0,GFR=0.0,Sodium=0.0,potassium=0.0,chloride=0.0,
    ...
    result = kidneyFuncyiontTest.kidneyFuncyiontTest(gender,Glucose,BUN,Creatinine,Calcium,GFR,Sodium,
                        potassium,chloride,CO2,Anion_Gap,BUN_Creat_ratio)
    return json.dumps(result, ensure_ascii=False).encode('utf8')

@app.route('/ESRBlood/<Blood_Clotting_Time_CT>/<Bleeding_Time_BT>')
def index14(Blood_Clotting_Time_CT=0.0,Bleeding_Time_BT=0.0):
    result = ESRBlood.BleedandclottingTime(Blood_Clotting_Time_CT,Bleeding_Time_BT)
    return json.dumps(result, ensure_ascii=False).encode('utf8')

@app.route('/ESRBlood/<PT>/<APTT>/<Fibrinogen>/<AT_III>/<FDPs>')
def index15(PT=0.0,APTT=0.0,Fibrinogen=0.0,AT_III=0.0,FDPs=0.0):
    result = ESRBlood.ProthrombinTime(PT,APTT,Fibrinogen,AT_III,FDPs)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index17(gender='',Haemoglobin=0.0,Haematocrit_PCV=0.0,RBCs_Count=0.0,MCV=0.0,MCH=0.0,MCHC=0.0,RDW_CV=0.0,
            Platelet_Count_EDTA_Blood=0.0,Total_Leucocytic_Count_EDTA_Blood=0.0,Neutrophils=0.0,Staff=0.0,Segmented=0.0,
            Lymphocytes=0.0,Monocytes=0.0,Eosinophils=0.0,Basophils=0.0):
    result = BloodDiseases.CompleteBloodPicture(gender,Haemoglobin,Haematocrit_PCV,RBCs_Count,MCV,MCH,MCHC,RDW_CV,
...
def index18(Hemoglobine_A=0.0,Hemoglobine_F=0.0,Hemoglobine_S=0.0,Hemoglobine_C=0.0):
    result = BloodDiseases.HbElectrophoresis(Hemoglobine_A,Hemoglobine_F,Hemoglobine_S,Hemoglobine_C)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index19(DCT_User_Result):
    if type(DCT_User_Result)==str:     
        ...
        result = BloodDiseases.DirectCoombsTest2(DCT_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
...
def index20(IDCT_User_Result):
    if type(IDCT_User_Result)==str:     
        ...
        result = BloodDiseases.InDirectCoombsTest2(IDCT_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح" 
...
def index21(RH_Result,BG=''):
    if type(RH_Result)==str:     
        ...
        result = BloodDiseases.BloodGroupANDRh2(RH_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح" 
...
def index22(gender='',G6P_User_Result=0.0):
    result = BloodDiseases.Glucose6phosphate(gender,G6P_User_Result)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index23(gender='',age='',Estrogen=0.0 ,Progesterone=0.0,FSH=0.0,LH=0.0,Prolactin=0.0,Testosteroe_Total=0.0,Testosteron_Free=0.0):
    result = Hormones.InfertilityTestsF(gender,age,Estrogen,Progesterone,FSH,LH,Prolactin,Testosteroe_Total,Testosteron_Free)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index24(gender='',Estrogen=0.0,FSH=0.0,LH=0.0,Prolactin=0.0,Testosteron_Total=0.0,Testosteron_Free=0.0):
    result = Hormones.InfertilityTestsM(gender,Estrogen,FSH,LH,Prolactin,Testosteron_Total,Testosteron_Free)
    return json.dumps(result, ensure_ascii=False).encode('utf8')

app.route('/Hormones/<TSH>/<Total_T4>/<Free_T4>/<Free_T3>/<T3>')
def index25(TSH=0.0,Total_T4=0.0,Free_T4=0.0,Free_T3=0.0,T3=0.0):
    result = Hormones.TSHTest(TSH,Total_T4,Free_T4,Free_T3,T3)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index32(Toxoplasma_IgM=0.0,Toxoplasma_IgG=0.0,Rubella_IgM=0.0,Rubella_IgG=0.0,
    ...
    result = TORCHIGg_IGM.TORCHIGgf(Toxoplasma_IgM,Toxoplasma_IgG,Rubella_IgM,Rubella_IgG,CMV_IgM,CMV_IgG,HSV_IgM,HSV_IgG)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index37(Karyotyping_User_Result=0.0):
    result = GeneticsAnalyses.Karyotyping(Karyotyping_User_Result)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index38(status='',Alanine=0.0,Alpha_amino_N_butyric_acid=0.0,Arginine=0.0,
            ...
            Carnosine='',Hydroxyproline='',i_methylhistidine=''):
    result = GeneticsAnalyses.Aminograminplasma(status,Alanine,Alpha_amino_N_butyric_acid,Arginine,
                        Asparagine,Aspartic_acid,Beta_alanine,Citrulline,Cystine,Glutamic_acid,Glutamine,Glycine,
                        Histidine,Isoleucine,Leucine,Lysine,Methionine,Ornithine,Phenylalanine,Proline,Serine,Taurine,
                        Threonine,Tyrosine,Valine,Alpha_aminoadipic_acid,Beta_amino_isobutyric_acid,Carnosine,Hydroxyproline,i_methylhistidine)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index40(Colour='',Odure='',Blood='',Consistency='',PH='',Worms='',Undigested_Food='',
    ...
    result = farmanalysis.StoolAnalysis(Colour,Odure,Blood,Consistency,PH,Worms,Undigested_Food,
         Epithelial_Cells, Protozoa,Helminths,Yeast_Cells,RBCs,Pus_Cells,Vegetable_cells,
                      Starch,Muscle_Fibers,Fat)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index41(RBCs=0.0,Pus_Cells=0.0,Bilharzial_Ova='',Trichomonas_vaginalis=''):
    result = farmanalysis.Prostatic(RBCs,Pus_Cells,Bilharzial_Ova,Trichomonas_vaginalis)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index42(Vaginal_User_result):
    if type(Vaginal_User_result)==str:     
        ...
        result = farmanalysis.VaginalExam2(Vaginal_User_result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index43(Color='',Method_of_production='',Abstinence=0.0,Volume=0.0,Liquefaction_Time=0.0,PH=0.0,Sperm_Count=0.0):
    result = farmanalysis.SemenExam(Color,Method_of_production,Abstinence,Volume,Liquefaction_Time,PH,Sperm_Count)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index44(Appearance='',Gram_Stain='',Pressure=0.0,Protein=0.0,Glucose=0.0,WCC=0.0,Glucose_serum_ratio=0.0):
    result = farmanalysis.CSFExam(Appearance,Gram_Stain,Pressure,Protein,Glucose,WCC,Glucose_serum_ratio)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index45(RPR,VDRL):
    ...
    for i in range(0,len(SyphilisL_User_Result)) :
        if type(SyphilisL_User_Result.get(SyphilisL[i]))==str:     
            ...
            result = AofSTdiseases.SyphilisTests2(RPR,VDRL)
            return json.dumps(result, ensure_ascii=False).encode('utf8')
        else:
            return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index46(HIV_AB_User_Result):
    if type(HIV_AB_User_Result)==str:     
        ...
        result = AofSTdiseases.AIDSTest2(HIV_AB_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index47(AbChlamydia_User_Result):
    if type(AbChlamydia_User_Result)==str:     
        ...
        result = AofSTdiseases.ChlamydiaTest2(AbChlamydia_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index48(HSV_User_Result):
    if type(HSV_User_Result)==str:     
        ...
        result = AofSTdiseases.HerpesSimplexVirusTest2(HSV_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index49(HBS_User_Result):
    if type(HBS_User_Result)==str:     
        ...
        result = AofSTdiseases.HepatitisBBloodTests2(HBS_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index50(TBS_User_Result):
    if type(TBS_User_Result)==str:     
        ...
        result = Tuberculosis.TBTest2(TBS_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index51(AN_User_Result):
    if type(AN_User_Result)==str:     
        ...
        result = ImmuneDiseases.AntinuclearAntibodyTest2(AN_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index52(ASMA_User_Result):
    if type(ASMA_User_Result)==str:     
        ...
        result = ImmuneDiseases.AntiSmoothMuscleAntibodyTest2(ASMA_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح" 
...
def index53(AMA_User_Result):
    if type(AMA_User_Result)==str:     
        ...
        result = ImmuneDiseases.AntiMitochondrialAntibodyTest2(AMA_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index54(ADNA_User_Result):
    if type(ADNA_User_Result)==str:     
        ...
        result = ImmuneDiseases.AntiDNATest2(ADNA_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index55(ALKM_User_Result):
    if type(ALKM_User_Result)==str:     
        ...
        result = ImmuneDiseases.AntiliverkidneyMicrosomalTest2(ALKM_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index56(ANCA_User_Result):
    if type(ANCA_User_Result)==str:     
        ...
        result = ImmuneDiseases.AntineutrophilCytoplasmicAntibodiesTest2(ANCA_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index57(AP_User_Result):
    if type(AP_User_Result)==str:     
        ...
        result = ImmuneDiseases.AntiPlateletABTest2(AP_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index58(AS_User_Result=0.0):
    result = ImmuneDiseases.AntiSpermABTest(AS_User_Result)
    return json.dumps(result, ensure_ascii=False).encode('utf8')

@app.route('/ImmuneDiseases/<gender>/<C3>/<C4>')
def index59(gender='',C3=0.0,C4=0.0):
    result = ImmuneDiseases.ComplementSystemTest(gender,C3,C4)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index60(IgE_Test,food=''):
    if type(IgE_Test)==str:     
        ...
        result = SensitivityAnalyses.FoodpanelallergenspecificTest2(food,IgE_Test)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index70(LCT_User_Result):
    if type(LCT_User_Result)==str:     
        ...
        result = TumorsTest.LungCancerTumorMarkersTest2(LCT_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"  
...
def index71(B_User_Result):
    if type(B_User_Result)==str:     
        ...
        result = TumorsTest.BladderTest2(B_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"
...
def index72(DS_User_Result):
    if type(DS_User_Result)==str:     
        ...
        result = DawnTest.DawnSyndromeTest2(DS_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"
...
def index73(AF_User_Result=0.0):
    result = DawnTest.AlphaFetoproteinTest(AF_User_Result)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index74(HIVAb_User_Result):
    if type(HIVAb_User_Result)==str:     
        ...
        result = AIDSTest.HumanImmunodeficiencyVirusAntibodiesTest2(HIVAb_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"
...
def index75(HIVPCR_User_Result=0.0):
    result = AIDSTest.HIVbyPCRTest(HIVPCR_User_Result)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def inde76(Cholesterol=0.0,Triglyceride=0.0,HDL_C=0.0,LDL_C=0.0):
    result = RiskEvaluationProfile.LipidProfile(Cholesterol,Triglyceride,HDL_C,LDL_C)
    return json.dumps(result, ensure_ascii=False).encode('utf8')

@app.route('/RiskEvaluationProfile/<H_User_Result>')
def index77(H_User_Result=0.0):
    result = RiskEvaluationProfile.HomocysteineTest(H_User_Result)
    return json.dumps(result, ensure_ascii=False).encode('utf8')

@app.route('/RiskEvaluationProfile/<AF_User_Result>')
def index78(AF_User_Result=0.0):
    result = RiskEvaluationProfile.ActivatedfactorXIITest(AF_User_Result)
    return json.dumps(result, ensure_ascii=False).encode('utf8')

@app.route('/RiskEvaluationProfile/<SE_User_Result>')
def index79(SE_User_Result=0.0):
    result = RiskEvaluationProfile.SerumFibrinogenTest(SE_User_Result)
    return json.dumps(result, ensure_ascii=False).encode('utf8')

@app.route('/RiskEvaluationProfile/<HRCRP_User_Result>')
def index80(HRCRP_User_Result=0.0):
    result = RiskEvaluationProfile.HighSensitivityCReactiveProteinTest(HRCRP_User_Result)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
...
def index91(LA_User_Result):
    if type(LA_User_Result)==str:     
        ...
        result = ThromboticRiskProfile.LupusAnticoagulantTest2(LA_User_Result)
        return json.dumps(result, ensure_ascii=False).encode('utf8')
    else:
        return "من فضلك تاكد من ادخال النتائج بشكل صحيح"
...
def index92(ACAb_User_Result=0.0):
    result = ThromboticRiskProfile.AntiCardiolipinAbsTest(ACAb_User_Result)
    return json.dumps(result, ensure_ascii=False).encode('utf8')
